...
%% decentralized control (fixed base load)
P_ev_a=zeros(numEV,T);
SOC_a=zeros(numEV,T);
for i=1:numEV
% P_ev=zeros(1,T);
cvx_begin
...
figure;
subplot(3,1,1);
plot(1:T, sum(P_ev_a,1) + baseLoad, 'b', 'LineWidth', 2); hold on;
plot(1:T, baseLoad, 'r--', 'LineWidth', 2);
xlabel('时间 (小时)'); ylabel('负荷 (kW)');
title('优化后总负荷'); legend('优化后', '基准负荷');

subplot(3,1,2);
imagesc(P_ev_a); colormap(jet); colorbar;
xlabel('时间 (小时)'); ylabel('EV编号');
title('EV 充放电策略');

subplot(3,1,3);
plot(1:T, SOC_a', 'k', 'LineWidth', 2);
xlabel('时间（小时）'); ylabel('电池能量');
title('EV能量变化');
